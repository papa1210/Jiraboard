...
Lưu ý vận hành
Chạy dev: docker compose -f docker-compose.dev.yml up --build
Chạy prod (đơn giản): docker compose -f docker-compose.prod.yml up --build -d
Prisma: trong container api, chạy migrate nếu chưa có schema: docker compose -f docker-compose.prod.yml run --rm api npx prisma migrate deploy
Đặt biến JWT_SECRET mạnh trong prod, và tùy chỉnh VITE_API_URL/CORS_ORIGIN nếu đổi host.

Đã thêm cấu hình Docker hóa theo cấu trúc thư mục hiện tại (client/, server/):

Dockerfile.api: build TS và chạy Node runtime cho server.
Dockerfile.client: build Vite và serve bằng Nginx.
nginx.conf: cấu hình SPA fallback.
docker-compose.dev.yml: dev stack với Postgres (volume), API (hot-reload), Vite dev server (5173), API 4000.
docker-compose.prod.yml: build images, Postgres + API + frontend Nginx (port 80), restart policy.
.env.example: mẫu biến môi trường.
Cách dùng:

Dev: docker compose -f docker-compose.dev.yml up --build
Prod: docker compose -f docker-compose.prod.yml up --build -d
API Imange: docker compose -f docker-compose.prod.yml build api
Chạy migrate trong prod (lần đầu): docker compose -f docker-compose.prod.yml run --rm api npx prisma migrate deploy
Nhớ đặt JWT_SECRET mạnh cho prod và điều chỉnh VITE_API_URL/CORS_ORIGIN nếu đổi host/proxy.
